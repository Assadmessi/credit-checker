...
const idInvalidCardCompaines = number => {
  const id = [];
  for(let k = 0; k < number.length; k++){
    let comapny = number[k];
    if(comapny[0] === 3){
      id.push('Amex');
    } else if(comapny[0] === 4){
      id.push('Visa');
...
console.log(idInvalidCardCompaines([invalid1]));
console.log(idInvalidCardCompaines([mystery2])); 
console.log(idInvalidCardCompaines(batch));